...
from .models import Question, Choice, Texts
...
class PollsView(generic.ListView):
    ...
    def get_queryset(self):
        return Question.objects.exclude(choice__isnull=True).filter(pub_date__lte=timezone.now()).union(Question.objects.filter(pub_date__lte=timezone.now(), question_type="Text"))
...
def Hlasovanie(request):
    asked_questions = Question.objects.exclude(choice__isnull=True).filter(pub_date__lte=timezone.now()).union(Question.objects.filter(pub_date__lte=timezone.now(), question_type="Text"))
    ...
    if ids == answered_questions:
        for question in asked_questions:
            if question.question_type == 'Choice':
                ...
                try:
                    choice = question.texts_set.get(text_text=request.POST.__getitem__(str(question.id)))
                    choice.votes += 1
                    choice.save()
                except (KeyError, Texts.DoesNotExist):
                    newtext = Texts(question=question, text_text=text, votes=1)
                    newtext.save()

    else:
        return render(request, 'polls/pollsview.html', {
            'object_list': asked_questions,
            'error_message': 'Nezaskrtol si volbu u otazok %s' % unanswered_questions,
        })
